"""
tests/test_parity4.py
---------------------
Parity check with a capped 500-bar window fed to evaluate() (keeps its slow
pivot loop tractable) and the full HTF series (so the 4H EMA matches). A
500-bar M15 window contains the current ET day in full and the recent swing
pivots, so evaluate()'s scalars equal strategy.feat's full-series values.

Run:  python tests/test_parity4.py
"""
