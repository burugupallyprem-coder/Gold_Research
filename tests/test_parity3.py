"""
tests/test_parity3.py
---------------------
Sparse-sample parity check: strategy.feat.decide_at vs strategy.evaluate.
Samples every Nth bar (full history window each) so evaluate()'s slow pivot
loop stays tractable while still covering many signal bars.

Run:  python tests/test_parity3.py
"""
...
def main(stride=7):
    ...
    n = len(df)
    lo, hi = min(400, n // 4), min(n - 1, 2400)
    ...
    print(f"sampled bars  : {compared} (stride {stride})")
