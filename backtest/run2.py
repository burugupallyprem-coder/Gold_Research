...
backtest/run2.py
----------------
Load cached OANDA candles, run the (fast, parity-verified) backtest, print
metrics, and write a blotter + equity curve + summary to reports/.

Usage:
    python backtest/run2.py
    python backtest/run2.py --disp 1.4 --threshold 70 --no-gate --tag test
    python backtest/run2.py --spread 0.5 --slippage 0.2 --tag stress
"""
...
def main():
    ...
    ap.add_argument("--disp", type=float, default=1.2)
    ...
    result = run_backtest_fast(df, htf, strat_cfg, risk_cfg, bt_cfg, cost=cost)
    metrics = compute_metrics(result.trades, result.equity_curve, bt_cfg.starting_equity)

    print("\n=== BACKTEST METRICS ===")
    print(json.dumps(metrics, indent=2, default=str))

    REPORTS.mkdir(exist_ok=True)
    trades_to_frame(result.trades).to_csv(REPORTS / f"blotter_{args.tag}.csv", index=False)
