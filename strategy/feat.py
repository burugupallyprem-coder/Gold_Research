...
strategy/feat.py
----------------
Vectorized single-pass precompute + scalar per-bar decision. Same signal logic
as strategy.evaluate(), computed once over the whole series so a multi-year
M15 backtest runs in seconds. tests/test_parity2.py proves bar-for-bar parity
with evaluate(). No look-ahead (see notes per column).
"""
